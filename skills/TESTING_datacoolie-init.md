...
### 1.1 Minimal project — defaults only

```powershell
& $py $skill/scripts/scaffold.py --name my_project --output /tmp/sc_basic
# ✓ Scaffolded project 'my_project' at /tmp/sc_basic
# Files created: .datacoolie/config.yaml, metadata/metadata.json,
#   metadata/connections.json, metadata/dataflows.json,
...
# Exit 0
Get-ChildItem /tmp/sc_basic -Recurse -File | Select-Object -ExpandProperty Name
Remove-Item -Recurse /tmp/sc_basic
```

### 1.2 Config.yaml engine and project_name match args

```powershell
& $py $skill/scripts/scaffold.py --name acme_etl --engine polars --output /tmp/sc_cfg
Get-Content /tmp/sc_cfg/.datacoolie/config.yaml | Select-String "acme_etl|polars"
# Should show project_name: acme_etl  and  default_engine: polars
Remove-Item -Recurse /tmp/sc_cfg
```

### 1.3 Source type: database — connections use sql format

```powershell
& $py $skill/scripts/scaffold.py --name db_proj --source-type database --output /tmp/sc_db
$c = Get-Content /tmp/sc_db/metadata/connections.json | ConvertFrom-Json
$c[0].format   # sql
$c[0].connection_type  # database
Remove-Item -Recurse /tmp/sc_db
```

### 1.4 Source type: api — connections use api format

```powershell
& $py $skill/scripts/scaffold.py --name api_proj --source-type api --output /tmp/sc_api
$c = Get-Content /tmp/sc_api/metadata/connections.json | ConvertFrom-Json
$c[0].format   # api
Remove-Item -Recurse /tmp/sc_api
```

### 1.5 Multiple source types (comma-separated)

```powershell
& $py $skill/scripts/scaffold.py --name multi_src --source-type file,database,api --output /tmp/sc_multi
# Exit 0 (primary source type is 'file', the rest inform config)
Remove-Item -Recurse /tmp/sc_multi
```

### 1.6 Multiple layers — one dataflow per layer

```powershell
& $py $skill/scripts/scaffold.py --name medal_proj --layers source2bronze,bronze2silver,silver2gold --output /tmp/sc_layers
...
Remove-Item -Recurse /tmp/sc_layers
```

### 1.7 Multiple platforms — config.yaml lists all

```powershell
& $py $skill/scripts/scaffold.py --name cloud_proj --platform local,aws,fabric --output /tmp/sc_plat
Get-Content /tmp/sc_cloud_proj/.datacoolie/config.yaml 2>$null
Get-Content /tmp/sc_plat/.datacoolie/config.yaml | Select-String "local|aws|fabric"
# Should show all three listed under platforms:
Remove-Item -Recurse /tmp/sc_plat
```

### 1.8 Destination format: parquet

```powershell
& $py $skill/scripts/scaffold.py --name pq_proj --dest-format parquet --output /tmp/sc_pq
$c = Get-Content /tmp/sc_pq/metadata/connections.json | ConvertFrom-Json
($c | Where-Object { $_.name -like "*destination*" }).format  # parquet
Remove-Item -Recurse /tmp/sc_pq
```

### 1.9 Destination format: iceberg

```powershell
& $py $skill/scripts/scaffold.py --name ice_proj --dest-format iceberg --output /tmp/sc_ice
$c = Get-Content /tmp/sc_ice/metadata/connections.json | ConvertFrom-Json
($c | Where-Object { $_.name -like "*destination*" }).format  # iceberg
Remove-Item -Recurse /tmp/sc_ice
```

### 1.10 Engine: spark — run_local.py uses SparkSession

```powershell
& $py $skill/scripts/scaffold.py --name spark_proj --engine spark --output /tmp/sc_spark
Get-Content /tmp/sc_spark/scripts/run_local.py | Select-String "SparkSession|SparkEngine"
# Should mention SparkSession or SparkEngine
Remove-Item -Recurse /tmp/sc_spark
```

### 1.11 Engine: mixed — prints advisory table

```powershell
& $py $skill/scripts/scaffold.py --name mix_proj --engine mixed --layers source2bronze,bronze2silver --output /tmp/sc_mix
# Output includes:
# Engine Advisory:
# ┌──────── ... Recommended ... Reason ──────────┐
# │ source2bronze (file) │ polars  │ File I/O ingest < 10GB typical   │
# │ bronze2silver (file) │ spark   │ Transforms with joins ...         │
Remove-Item -Recurse /tmp/sc_mix
```

### 1.12 Mixed engine — config.yaml contains engine_strategy.overrides

```powershell
& $py $skill/scripts/scaffold.py --name mix2 --engine mixed --layers source2bronze,bronze2silver --output /tmp/sc_mix2
Get-Content /tmp/sc_mix2/.datacoolie/config.yaml | Select-String "overrides|source2bronze|bronze2silver"
# Should show overrides section with per-layer engine assignments
Remove-Item -Recurse /tmp/sc_mix2
```

### 1.13 Generated metadata.json passes validate

```powershell
& $py $skill/scripts/scaffold.py --name valid_test --output /tmp/sc_valid
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/sc_valid/metadata/metadata.json
# ✓ metadata.json is valid (schema v0.1.0)
Remove-Item -Recurse /tmp/sc_valid
```

### 1.14 Functions package is buildable (has pyproject.toml + __init__.py)

```powershell
& $py $skill/scripts/scaffold.py --name funcs_test --output /tmp/sc_funcs
Test-Path /tmp/sc_funcs/functions/pyproject.toml  # True
Test-Path /tmp/sc_funcs/functions/__init__.py      # True
Test-Path /tmp/sc_funcs/functions/sources.py       # True
Remove-Item -Recurse /tmp/sc_funcs
```

### 1.15 Invalid source type — expect exit 2

```powershell
& $py $skill/scripts/scaffold.py --name bad_src --source-type kafka --output /tmp/sc_bad
# ERROR: Invalid source type 'kafka'. Valid: ['api', 'database', 'file']
# Exit 2
echo "Exit: $LASTEXITCODE"  # Exit: 2
```

### 1.16 Invalid platform — expect exit 2

```powershell
& $py $skill/scripts/scaffold.py --name bad_plat --platform gcp --output /tmp/sc_bad2
...
```powershell
& $py $skill/scripts/scaffold.py --name bad_layer --layers raw2bronze --output /tmp/sc_bad3
...
```powershell
New-Item -ItemType Directory -Force /tmp/sc_exists | Out-Null
'existing' | Set-Content /tmp/sc_exists/README.md
& $py $skill/scripts/scaffold.py --name exists_test --output /tmp/sc_exists 2>&1
...
# Exit 0
Remove-Item -Recurse /tmp/sc_exists
```
...
```powershell
New-Item -ItemType Directory -Force /tmp/intro_data/orders | Out-Null
New-Item -ItemType Directory -Force /tmp/intro_data/customers | Out-Null
...
```powershell
& $py $skill/scripts/introspect.py --mode folder --path /tmp/intro_data --connection-name raw_data
# Outputs valid JSON to stdout
& $py $skill/scripts/introspect.py --mode folder --path /tmp/intro_data --connection-name raw_data | ConvertFrom-Json
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode folder --path /tmp/intro_data --connection-name src | ConvertFrom-Json
...
```powershell
& $py $skill/scripts/introspect.py --mode folder --path /tmp/intro_data --connection-name src --output /tmp/intro_out.json
Test-Path /tmp/intro_out.json   # True
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode folder --path /tmp/intro_data --connection-name src --dest-connection bronze_layer --dest-format parquet | ConvertFrom-Json
...
```powershell
New-Item -ItemType Directory -Force /tmp/flat_data | Out-Null
@"
id,value
1,hello
2,world
...
$meta = & $py $skill/scripts/introspect.py --mode folder --path /tmp/flat_data --connection-name flat_src | ConvertFrom-Json
$meta.dataflows.Count  # 1
$meta.dataflows[0].name  # ingest_flat_data
Remove-Item -Recurse /tmp/flat_data
```

### 2.7 JSON files — schema inferred from first record

```powershell
New-Item -ItemType Directory -Force /tmp/json_data/events | Out-Null
'[{"event_id":1,"amount":19.99,"created_at":"2024-01-01","is_paid":true}]' | Set-Content /tmp/json_data/events/events.json

$meta = & $py $skill/scripts/introspect.py --mode folder --path /tmp/json_data --connection-name json_src | ConvertFrom-Json
...
Remove-Item -Recurse /tmp/json_data
```

### 2.8 Path is not a directory — expect exit 2

```powershell
& $py $skill/scripts/introspect.py --mode folder --path /tmp/nonexistent_dir --connection-name src
...
```powershell
New-Item -ItemType Directory -Force /tmp/empty_dir | Out-Null
& $py $skill/scripts/introspect.py --mode folder --path /tmp/empty_dir --connection-name src
# Exit 1 (no recognizable data files)
Remove-Item -Recurse /tmp/empty_dir
```

### 2.10 --path missing for folder mode — expect exit 2

```powershell
& $py $skill/scripts/introspect.py --mode folder --connection-name src
...
```powershell
Remove-Item -Recurse /tmp/intro_data
```
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/schema.sql --connection-name db_src | ConvertFrom-Json
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/schema.sql --connection-name db_src --dest-connection silver --dest-format iceberg | ConvertFrom-Json
...
```powershell
& $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/schema.sql --connection-name db_src --output /tmp/ddl_out.json
Test-Path /tmp/ddl_out.json  # True
...
```powershell
& $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/missing.sql --connection-name src
...
```powershell
& $py $skill/scripts/introspect.py --mode ddl --connection-name src
...
$meta = & $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/ifnotexists.sql --connection-name src | ConvertFrom-Json
...
$meta = & $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/empty.sql --connection-name src | ConvertFrom-Json
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "id:int,name:string,amount:decimal,created_at:timestamp" | ConvertFrom-Json
$hints = $meta.dataflows[0].transform.schema_hints
$hints.Count  # 4
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "a:int,b:string,c:decimal" | ConvertFrom-Json
$hints = $meta.dataflows[0].transform.schema_hints
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "user_id:bigint,label:varchar,score:float,event_date:datetime" | ConvertFrom-Json
$hints = $meta.dataflows[0].transform.schema_hints
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "id:int,notes" | ConvertFrom-Json
$hints = $meta.dataflows[0].transform.schema_hints
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "id:int" --connection-name my_src | ConvertFrom-Json
...
```powershell
$meta = & $py $skill/scripts/introspect.py --mode manual --columns "id:int" --dest-connection landing | ConvertFrom-Json
...
```powershell
& $py $skill/scripts/introspect.py --mode manual --connection-name src
...
```powershell
& $py $skill/scripts/introspect.py --mode manual --columns "id:int,name:string" --output /tmp/manual_out.json
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/manual_out.json
...
### 5.1 Scaffold → validate

```powershell
& $py $skill/scripts/scaffold.py --name e2e_proj --output /tmp/e2e_test
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/e2e_test/metadata/metadata.json
# ✓ metadata.json is valid (schema v0.1.0)
& $py skills/datacoolie-metadata/scripts/lint.py /tmp/e2e_test/metadata/metadata.json --env dev
# ✓ metadata.json: no lint warnings  (or INFO only)
Remove-Item -Recurse /tmp/e2e_test
```

### 5.2 Scaffold → deploy preflight

```powershell
& $py $skill/scripts/scaffold.py --name deploy_ready --output /tmp/deploy_ready
& $py skills/datacoolie-deploy/scripts/preflight.py --platform local --project-dir /tmp/deploy_ready
# ✓ Metadata found (1 file(s) in metadata/)
# ✓ Functions packageable (functions/pyproject.toml found (wheel build))
# Exit 0
Remove-Item -Recurse /tmp/deploy_ready
```

### 5.3 Folder introspect → validate

```powershell
New-Item -ItemType Directory -Force /tmp/e2e_intro/sales | Out-Null
"id,revenue,sale_date`n1,100.0,2024-01-01" | Set-Content /tmp/e2e_intro/sales/sales.csv
& $py $skill/scripts/introspect.py --mode folder --path /tmp/e2e_intro --connection-name raw --output /tmp/e2e_intro_out.json
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/e2e_intro_out.json
# ✓ e2e_intro_out.json is valid (schema v0.1.0)
Remove-Item -Recurse /tmp/e2e_intro
...
& $py $skill/scripts/introspect.py --mode ddl --ddl-file /tmp/e2e_ddl.sql --connection-name db --output /tmp/e2e_ddl_out.json
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/e2e_ddl_out.json
...
```powershell
& $py $skill/scripts/introspect.py --mode manual --columns "id:int,name:string,price:decimal" --output /tmp/e2e_manual_out.json
& $py skills/datacoolie-metadata/scripts/validate.py /tmp/e2e_manual_out.json
