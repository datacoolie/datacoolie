...
- [ ] Placeholder values map to `.datacoolie/config.yaml` structure
...
| Missing metadata | Empty `metadata/` | AI stops and recommends running `datacoolie-metadata` |
| Missing secret env var | Metadata includes `secrets_ref` | AI lists missing variables and stops |
| No functions dir | Remove `functions/` | AI warns and continues |

#### 3.2 Runner Generation

| Platform | Prompt | Verify |
|----------|--------|--------|
| Local | Generate local runner | `.datacoolie/generated/run_local.py` exists and is valid Python |
...
- Setup: `.datacoolie/config.yaml` and at least one metadata file exist
- Prompt: Deploy to prod
- Verify: AI reads existing config and metadata without asking for platform basics

### 4.2 Missing config.yaml

- Setup: metadata exists, `.datacoolie/config.yaml` absent
- Prompt: Deploy
- Verify: AI asks for platform, engine, and environment config interactively
...
- Setup: config + metadata exist, `functions/` absent
